...
% -------------------------------------------------------------------------
    ...
    %======================================================================
        ...
        if abs(xyz(d)-nxyz(d))>0
            ...
            tfce_results_ui('UpdateSPMval',UD)
        end
    ...
    %======================================================================
        % tfce_results_ui('UpdateSPMval',hFxyz)
        % tfce_results_ui('UpdateSPMval',UD)
    ...
    %======================================================================
        % xyz = tfce_results_ui('GetCoords',hFxyz)
        if nargin<2, hFxyz='Interactive'; else hFxyz=varargin{2}; end
        hFxyz     = tfce_results_ui('FindXYZframe',hFxyz);
    ...
    %======================================================================
        % [xyz,d] = tfce_results_ui('SetCoords',xyz,hFxyz,hC)
        if nargin<4, hC=NaN; else hC=varargin{4}; end
        if nargin<3, hFxyz=tfce_results_ui('FindXYZframe'); else hFxyz=varargin{3}; end
    ...
    %======================================================================
        % hFxyz = tfce_results_ui('FindXYZframe',h)
    ...
    %======================================================================
        ...
        h4 = uicontrol('Parent',hGraphUIButtsF,'Style','popupmenu',...
            ...
            'Callback','tfce_results_ui(''PlotUiCB'')',...
            'Interruptible','on','Enable','on',...
        ...
        set(hGraphUIbg,'UserData',...
            ...
            'DeleteFcn','tfce_results_ui(''Delete'',get(gcbo,''UserData''))')
    ...
    %======================================================================
        % Fgraph = tfce_results_ui('Clear',F,mode)
    ...
    %======================================================================
        % hMP = tfce_results_ui('LaunchMP',M,DIM,hReg,hBmp)
        ...
        set(hBmp,'DeleteFcn','tfce_results_ui(''delete'',get(gcbo,''UserData''))')
...
if ~isempty(xSPM.thresDesc)
    if strcmp(xSPM.STAT,'P')
        % These are soon overwritten by tfce_getSPM
...
[hReg,xSPM,SPM] = tfce_results_ui('setup',xSPM2);
TabDat = tfce_list('List',xSPM,hReg);
